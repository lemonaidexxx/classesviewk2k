...
Open **K2K Data Tracker → Sheet5**. Keep headers on row 1 (current layout) or row 2 (legacy layout). One row is one class or cohort; two batches of the same course need separate rows.
...
| Notes / Additional Notes | Current layout: Notes in P, Additional Notes in unlabeled Q. Legacy layout: Notes in O, Additional Notes in unlabeled P. Preserve both columns. |
...
The helper detects headers on row 1 or 2, checks headers, appends missing optional columns after all occupied columns (including reserved Q in the current layout or P in the legacy layout), assigns UUID Class IDs only where missing, creates missing Courses and Dashboard_Settings tabs, and supplies missing defaults. It preserves existing values/formulas and adds non-rejecting course/archive dropdowns. Duplicate IDs are logged for owner review rather than silently changed.
...
`Projected Class Opening`, `Class Opening`, and `Planned Opening` all feed the dashboard�s planned opening date. Explicit Additional Notes headers take precedence over the unlabeled-column fallback.
